	...
	<div class="row">
		<div class="col l6 offset-l3 m8 offset-m2">
			...
			<div id="tab2" class="row">
				<form action="{{ route('admin.galleries.photo', $item->id) }}" method="POST" class="dropzone teal lighten-5" id="photos-dropzone">
                    {{ csrf_field() }}
                        ...
                        <tbody>
                            @foreach($item->photos as $photo)
                                <tr>
                                    ...
                                    <td>{{ $photo->name }}</td>
                                    <td>{{ $photo->order }}</td>
                                    <td><button onclick="confirmDelete(this, '{{ $photo->id }}', '{{ route('admin.galleries.photo.delete', [$item->id, $photo->id]) }}')" class="btn btn-small waves-effect waves-light red darken-2"><i class="material-icons">delete</i></button></td>
                                </tr>
                            @endforeach
                        </tbody>
                    </table>
