...
@section('content')
    <h4 class="center">Высота верхних шкафов</h4>
    <p><a href="{{ route('admin.cupboard_heights.create') }}" class="btn waves-effect waves-light"><i class="material-icons left">add_circle</i> Добавить</a></p>
    ...
    @if ($items->count())
        <table id="table_items">
            ...
            <tfoot>
                <tr>
                    <th colspan="4" class="pager form-inline">
            ...
            <tbody>
                @foreach($items as $item)
                    <tr>
                        <td>{{ $item->id }}</td>
                        <td>{{ $item->name }}</td>
                        <td><a href="{{ route('admin.cupboard_heights.edit', $item->id) }}" class="btn btn-small waves-effect waves-light"><i class="material-icons">edit</i></a></td>
